...
describe("ERC20TokenA", function () {
  ...
  it("permit ", async function () {
    ...
    expect(await tokens[i].contract.balanceOf(user1.address)).to.be.equal(mintAmount);
     let nonce = await tokens[i].contract.connect(user1).nonces(user1.address);
    ...
    let domainData = {name:tokens[i].name,version:tokens[i].version,chainId:netId,verifyingContract:tokens[i].contract.address}
    const message = {
      owner: user1.address,
    ...
    await tokens[i].contract.connect(user2).permit(user1.address, stakeContract.address, mintAmount, deadline, v,r,s);
    expect(await tokens[i].contract.allowance(user1.address, stakeContract.address)).to.be.equal(mintAmount);
    await stakeContract.connect(user1).stake(mintAmount);
    expect(await stakeContract.stakeAmounts(user1.address)).to.be.equal(mintAmount);
