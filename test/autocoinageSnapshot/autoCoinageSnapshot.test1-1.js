...
describe("AutoCoinageSnapshot - test1", function () {
  ...
  it("2. staking ", async function () {
    ...
    await autoCoinageSnapshot.connect(admin).addSync(layer2s[0], admin.address);
    await checkList(autoCoinageSnapshot, layer2Address, accounts)

  });

  it("2-1. snapshot ", async function () {
    let tx = await autoCoinageSnapshot.connect(admin)["snapshot()"]();
    await checkList(autoCoinageSnapshot, layer2Address, accounts)

  });

  it("3. requestWithdrawal ", async function () {
      const tonAmount =  ethers.utils.parseUnits("100", "ether");
      await DepositManager.connect(user1).requestWithdrawal(layer2s[0], tonAmount);
      //await autoCoinageSnapshot.connect(admin).syncBatch(layer2s[0], [user1.address]);
      await autoCoinageSnapshot.connect(admin).addSync(layer2s[0], user1.address);
      await checkList(autoCoinageSnapshot, layer2Address, accounts)
  });

  it("3-1. snapshot ", async function () {
    let tx = await autoCoinageSnapshot.connect(admin)["snapshot()"]();
    await checkList(autoCoinageSnapshot, layer2Address, accounts)

  });

  it("4. updateSeigniorage ", async function () {
  ...
  it("4-1. snapshot ", async function () {
    let tx = await autoCoinageSnapshot.connect(admin)["snapshot()"]();
    await checkList(autoCoinageSnapshot, layer2Address, accounts)

  });

  it("5. sync ", async function () {
  ...
  it("5-1. snapshot ", async function () {
    let tx = await autoCoinageSnapshot.connect(admin)["snapshot()"]();
    await checkList(autoCoinageSnapshot, layer2Address, accounts)

  });

  it("6. requestWithdrawalAll ", async function () {
    await DepositManager.connect(admin).requestWithdrawalAll(layer2s[0]);
    await autoCoinageSnapshot.connect(admin).addSync(layer2s[0], admin.address);
    await checkList(autoCoinageSnapshot, layer2Address, accounts)
  });

  it("6-1. snapshot ", async function () {
    let tx = await autoCoinageSnapshot.connect(admin)["snapshot()"]();
    await checkList(autoCoinageSnapshot, layer2Address, accounts)
...
async function checkSyncAccountInLayer2(autoCoinageSnapshot, layer2, account){
    ...
    if(adminInfo.snapshotted){
      if( adminInfo.snapShotBalance.toString() != adminInfo.curBalances.toString() ) {
        ...
        //console.log("snapShotBalance sync");
      }
      if( adminInfo.snapShotRefactoredCount.toString() != adminInfo.curRefactoredCounts.toString() ){
        console.log("snapShotRefactoredCount not sync");
      } else {
        //console.log("snapShotRefactoredCount sync");
      }
      if( adminInfo.snapShotRemain.toString() != adminInfo.curRemains.toString() ) {
        console.log("snapShotRemain not sync");
      } else {
        //console.log("snapShotRemain sync");
      }
    } else {
...
async function checkSyncFactor(autoCoinageSnapshot, layer2){
    ...
    if(adminInfo.snapshotted){
      if( adminInfo.snapShotFactor.toString() != adminInfo.curFactor.toString() ) {
        console.log("snapShotFactor not sync");
      } else {
       // console.log("snapShotFactor sync");
      }
      if( adminInfo.snapShotRefactorCount.toString() != adminInfo.curRefactorCount.toString() ) {
        console.log("snapShotRefactorCount not sync");
      } else {
       // console.log("snapShotRefactorCount sync");
      }
...
async function checkSyncTotalAmount(autoCoinageSnapshot){
  ...
  if( total.toString() != amountTokamak.toString() ){
    console.log("totalSupply not sync");
  } else {
   // console.log("totalSupply sync");
  }
}
