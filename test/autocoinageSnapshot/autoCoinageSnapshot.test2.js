...
describe("AutoCoinageSnapshot - test2", function () {
  ...
  it("4. snapshot ", async function () {
    let tx = await autoCoinageSnapshot.connect(admin)["snapshot()"]();

    await checkList(autoCoinageSnapshot, layer2Address, accounts)

  });
  it("5. 스냅샷 어그리게이터 저장 ", async function () {

    let id =  await autoCoinageSnapshot.snashotAggregatorTotal();

    // console.log('current');

    let getCurrentLayer2SnapshotIdLayer2 =  await autoCoinageSnapshot["getCurrentLayer2SnapshotId(address)"](layer2Address);
    // console.log('getCurrentLayer2SnapshotIdLayer2', getCurrentLayer2SnapshotIdLayer2);

    let totalSupply =  await autoCoinageSnapshot["totalSupply()"]();
    let balanceOfLayer2 =  await autoCoinageSnapshot["balanceOf(address,address)"](layer2Address, admin.address);
    let balanceOf =  await autoCoinageSnapshot["balanceOf(address)"](admin.address);
    // console.log('totalSupply()',totalSupply);
    // console.log('balanceOf(address,address)',balanceOfLayer2);
    // console.log('balanceOf(address)',balanceOf);


    // console.log('id',id);
    let totalSupplyAt =  await autoCoinageSnapshot["totalSupplyAt(uint256)"](id);
    let balanceOfAt =  await autoCoinageSnapshot["balanceOfAt(address,uint256)"](admin.address, id);
    let balanceOfLayer2At =  await autoCoinageSnapshot["balanceOfAt(address,address,uint256)"](layer2Address, admin.address,getCurrentLayer2SnapshotIdLayer2);
    // console.log('totalSupplyAt',totalSupplyAt);
    // console.log('balanceOf(address,address) At',balanceOfLayer2At);
    // console.log('balanceOf(address)At',balanceOfAt);
  ...
  it("5. 스냅샷 어그리게이터로 조회 ", async function () {
    ...
    for(let i=0; i< snapshotAggregatorIds.length; i++){
      ...
      let balanceOf =  await autoCoinageSnapshot["balanceOfAt(address,uint256)"](admin.address, snapshotAggregatorIds[i]);

      // console.log(i, totalSupplys[i], totalSupply);
      // console.log(i, balanceOfs[i], balanceOf);
