...
task("deploy-autocoinage-snapshot", "")
  ...
  .setAction(async function ({
    ...
    //   "0x56BC75E2D63100000",
    // ]);
    const AutoCoinageSnapshot = await ethers.getContractFactory(
      "AutoCoinageSnapshot"
    );
    const autoCoinageSnapshot = await AutoCoinageSnapshot.connect(
      admin
    ).deploy();
    ...
    const AutoCoinageSnapshotProxy = await ethers.getContractFactory(
      "AutoCoinageSnapshotProxy"
    );
    const autoCoinageSnapshotProxy = await AutoCoinageSnapshotProxy.connect(
      admin
    ).deploy();
    await autoCoinageSnapshotProxy.deployed();

    console.log(
      "AutoCoinageSnapshotProxy Deployed:",
    ...
    await (
      await autoCoinageSnapshotProxy
        .connect(admin)
        .upgradeTo(autoCoinageSnapshot.address)
    ).wait();
    await (
      await autoCoinageSnapshotProxy
        .connect(admin)
        .setNameSymbolDecimals(name, symbol, 27)
    ).wait();

    const AutoCoinageSnapshotABI = JSON.parse(
      await fs.readFileSync("./abi/AutoCoinageSnapshot.json")
    ).abi;
    ...
    await (
      await autoCoinageSnapshotContract
        .connect(admin)
        .setAddress(seigManagerAddress, layer2RegistryAddress)
    ).wait();
...
task("deploy-autocoinage-snapshot2", "")
  ...
  .setAction(async function ({
    ...
    try {
      ...
      console.log(
        "AutoCoinageSnapshot2 Deployed:",
        autoCoinageSnapshot.address
      );
      ...
      console.log(
        "AutoCoinageSnapshotProxy2 Deployed:",
        autoCoinageSnapshotProxy.address
      );
      ...
      const AutoCoinageSnapshotABI = JSON.parse(
        await fs.readFileSync("./abi/AutoCoinageSnapshot2.json")
      ).abi;
      const autoCoinageSnapshotContract = new ethers.Contract(
        autoCoinageSnapshotProxy.address,
        AutoCoinageSnapshotABI,
...
task("upgrade-autocoinage-snapshot", "")
  ...
  .setAction(async function ({ autoCoinageAddress }) {
    ...
    await autoCoinageSnapshot.deployed();
    console.log(
      "AutoCoinageSnapshotImp Deployed:",
      autoCoinageSnapshot.address
    );

    const AutoCoinageSnapshotProxyABI = JSON.parse(
      await fs.readFileSync("./abi/AutoCoinageSnapshotProxy.json")
    ).abi;
    const AutoCoinageSnapshotABI = JSON.parse(
      await fs.readFileSync("./abi/AutoCoinageSnapshot2.json")
    ).abi;
...
task("deploy-erc20-recorder", "")
  ...
  .setAction(async function ({ ownerAddress, depositManagerAddress }) {
    ...
    const tokenRecorderContract = await ethers.getContractFactory(
      "ERC20Recorder"
    );
    tokenRecorder = await tokenRecorderContract
      .connect(admin)
      .deploy("TONStakedRatio", "TSR", admin.address, depositManagerAddress);
...
task("deploy-token-dividend-pool", "")
  ...
  .setAction(async function ({ erc20RecorderAddress }) {
    ...
    const TokenDividendPoolImplContract = await ethers.getContractFactory(
      "TokenDividendPool"
    );
    const dividendPoolImpl = await TokenDividendPoolImplContract.connect(
      admin
    ).deploy();
    await dividendPoolImpl.deployed();

    const TokenDividendPoolProxyContract = await ethers.getContractFactory(
      "TokenDividendPoolProxy"
    );
    const dividendPoolProxy = await TokenDividendPoolProxyContract.connect(
      admin
    ).deploy(dividendPoolImpl.address, admin.address);
    await dividendPoolProxy.deployed();
    await (
      await dividendPoolProxy.connect(admin).initialize(erc20RecorderAddress)
    ).wait();

    // dividendPool = await ethers.getContractAt("TokenDividendPool", dividendPoolProxy.address);

    console.log(
      "Token Dividend Pool Implementation Deployed:",
      dividendPoolImpl.address
    );
    console.log(
      "Token Dividend Pool Proxy Deployed:",
      dividendPoolProxy.address
    );
...
task("deploy-power-ton-swapper", "")
  ...
  .setAction(async function ({
    ...
    const PowerTONSwapperImplContract = await ethers.getContractFactory(
      "PowerTONSwapper"
    );
    const powerTONSwapperImpl = await PowerTONSwapperImplContract.connect(
      admin
    ).deploy();
    ...
    const PowerTONSwapperProxyContract = await ethers.getContractFactory(
      "PowerTONSwapperProxy"
    );
    const powerTONSwapperProxy = await PowerTONSwapperProxyContract.connect(
      admin
    ).deploy(
      powerTONSwapperImpl.address,
      wtonAddress,
      tosAddress,
      uniswapRouterAddress,
      autocoinageSnapshotAddress,
    ...
    // ---------------
    const AutoCoinageSnapshotProxyABI = JSON.parse(
      await fs.readFileSync("./abi/AutoCoinageSnapshotProxy2.json")
    ).abi;
    const autoCoinageSnapshotProxy = new ethers.Contract(
      autocoinageSnapshotAddress,
      AutoCoinageSnapshotProxyABI,
    ...
    const tx = await autoCoinageSnapshotProxy
      .connect(admin)
      .grantRole(ADMIN_ROLE, powerTONSwapperProxy.address);
...
task("set-erc20-recorder", "")
  ...
  .setAction(
    async ({
      ...
      const [admin] = await ethers.getSigners();
      const erc20Recorder = await ethers.getContractAt(
        "ERC20Recorder",
        erc20RecorderAddress
      );
      ...
      const tx = await erc20Recorder
        .connect(admin)
        .grantRole(MINTER_ROLE, powerTonAddress);
      const tx1 = await erc20Recorder
        .connect(admin)
        .grantRole(BURNER_ROLE, powerTonAddress);
      const tx2 = await erc20Recorder
        .connect(admin)
        .grantRole(SNAPSHOT_ROLE, tokenDividendPoolAddress);
...
task("get-sum-of-layer2s", "")
  ...
  .setAction(async ({ layer2RegistryAddress, seigManagerAddress }) => {
    ...
    const seigManagerABI = JSON.parse(
      await fs.readFileSync("./abi/seigManager.json")
    ).abi;
    ...
    const layer2RegistryABI = JSON.parse(
      await fs.readFileSync("./abi/layer2Registry.json")
    ).abi;
    ...
    const autoRefactorCoinageABI = JSON.parse(
      await fs.readFileSync("./abi/autoRefactorCoinage.json")
    ).abi;

    const num = await layer2Registry.numLayer2s();
    let total = ethers.BigNumber.from(0);

    console.log("layer2Registry num", num);
    for (let i = 0; i < num; ++i) {
      console.log("-----------------");
      const layer2Address = await layer2Registry.layer2ByIndex(i);
      ...
      const coinageTotalSupply = await coinage.totalSupply();
    ...
    const totalStakedWei = ethers.utils.formatUnits(total, 9);
    const end = Math.min(totalStakedWei.indexOf("."), totalStakedWei.length);
    console.log(
      "sum of coinageTotalSupply (wei) ",
      totalStakedWei.substring(0, end)
    );
  });
...
task("compare-layer2-staked-amount", "")
  ...
  .setAction(async ({ layer2RegistryAddress, seigManagerAddress }) => {
    ...
    const stakers = JSON.parse(
      await fs.readFileSync("./data/stakers-finish.json")
    );
    const stakesOfAllUsers = JSON.parse(
      await fs.readFileSync("./data/stakesOfAllUsers-update.json")
    );
    const autoRefactorCoinageABI = JSON.parse(
      await fs.readFileSync("./abi/autoRefactorCoinage.json")
    ).abi;
      ...
      for (const staker of stakers) {
        ...
        if (stakesOfAllUsers[layer2][staker]) {
          totalStaked = totalStaked.add(
            ethers.BigNumber.from(stakesOfAllUsers[layer2][staker])
          );
        }
      }
...
task(
  "display-owner-autocoinage-powerton-dividendpool",
  "Display the owner of autocoinage, powerton and dividendpool"
)
  ...
  .setAction(async function ({
    ...
    const autoCoinageSnapshot2 = await ethers.getContractAt(
      "AutoCoinageSnapshot2",
      autoCoinageSnapshotAddress
    );
    const powerTONSwapper = await ethers.getContractAt(
      "PowerTONSwapper",
      powerTonAddress
    );
    const tokenDividendPool = await ethers.getContractAt(
      "TokenDividendPool",
      dividendPoolAddress
    );

    const hasRoleDEFAULT_ADMIN_ROLE = await autoCoinageSnapshot2
      .connect(admin)
      .hasRole(DEFAULT_ADMIN_ROLE, ownerAddress);
    console.log(
      "AutoCoinageSnapshot2 ownerAddress hasRole(DEFAULT_ADMIN_ROLE) :",
      hasRoleDEFAULT_ADMIN_ROLE
    );
    const hasRoleADMIN_ROLE = await autoCoinageSnapshot2
      .connect(admin)
      .hasRole(ADMIN_ROLE, ownerAddress);
    console.log(
      "AutoCoinageSnapshot2 ownerAddress hasRole(ADMIN_ROLE) :",
      hasRoleADMIN_ROLE
    );

    const isAdmin0 = await powerTONSwapper.connect(admin).isAdmin(ownerAddress);
    console.log("PowerTONSwapper ownerAddress isAdmin :", isAdmin0);

    const isAdmin1 = await tokenDividendPool
      .connect(admin)
      .isAdmin(ownerAddress);
    console.log("TokenDividendPool ownerAddress isAdmin :", isAdmin1);
  });

task(
  "change-owner-autocoinage-powerton-dividendpool",
  ...
  .setAction(async function ({
    ...
    const tx = await autoCoinageSnapshot2
      .connect(admin)
      .addOwner(newOwnerAddress);
    await tx.wait();
    console.log(
      "AutoCoinageSnapshotProxy2 addOwner(newOwnerAddress):  tx:",
      tx.hash
    );

    const tx1 = await powerTONSwapper
      .connect(admin)
      .transferAdmin(newOwnerAddress);
    await tx1.wait();
    console.log(
      "PowerTONSwapper transferAdmin(",
      newOwnerAddress,
      ") : tx:",
      tx1.hash
    );

    const tx2 = await tokenDividendPool
      .connect(admin)
      .transferAdmin(newOwnerAddress);
    await tx2.wait();
    console.log(
      "TokenDividendPool transferAdmin(",
      newOwnerAddress,
      ") : tx:",
      tx1.hash
    );
...
task("renounce-owner-autocoinage", "renounce the owner of AutocoinageSnapshot2")
  ...
  .setAction(async function ({ autoCoinageSnapshotAddress }) {
    ...
    const tx1 = await autoCoinageSnapshot2
      .connect(admin)
      .renounceRole(ADMIN_ROLE, admin.address);
    console.log(
      "AutoCoinageSnapshotProxy2 renounceRole(ADMIN_ROLE) :",
      tx1.hash
    );
    ...
    const hasRole1 = await autoCoinageSnapshot2
      .connect(admin)
      .hasRole(ADMIN_ROLE, admin.address);
    console.log(
      "ERC20AFactory ",
      admin.address,
      " hasRole(ADMIN_ROLE) :",
    ...
    const hasRole = await autoCoinageSnapshot2
      .connect(admin)
