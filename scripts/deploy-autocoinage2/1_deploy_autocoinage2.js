...
async function main() {
  ...
  const AutoCoinageSnapshot = await hre.ethers.getContractFactory(
    "AutoCoinageSnapshot2"
  );
  const autoCoinageSnapshot = await AutoCoinageSnapshot.deploy();

  const tx = await autoCoinageSnapshot.deployed();

  console.log("tx:", tx.deployTransaction.hash);
  console.log("AutoCoinageSnapshot2 deployed to:", autoCoinageSnapshot.address);
